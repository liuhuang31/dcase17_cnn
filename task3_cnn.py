...
# -*- coding: utf-8 -*-
#
# DCASE 2017::Sound Event Detection in Real-life Audio / CNN based system 
...
def main(argv):
    ...
    title("DCASE 2017::Sound Event Detection in Real-life Audio")
...
def do_system_training(dataset, model_path, feature_normalizer_path, feature_path, hop_length_seconds, classifier_params,
    ...
    for fold in [4,3,2,1]:
                    ...
                    if classifier_method == 'cnn':
                        ...
                        for i in range(0, feature_data.shape[0] - wd, classifier_params['step']):
                            ...
                            if scene_label == 'street':
                              ...
                              #numpy.concatenate( (feature_data[i: i + wd,:], numpy.full((wd, 1), -1.0)) , axis=1)
        ...
        if classifier_method == 'cnn':
            ...
            # same the same model for street
            current_model_file = get_model_filename(fold=fold, scene_label='street', path=model_path)
            model_container['test_dic'] = dic_street
            save_data(current_model_file, model_container)
...
def do_system_evaluation(dataset, result_path, dataset_evaluation_mode='folds'):
    ...
    for scene_id, scene_label in enumerate(dataset.scene_labels):
        ...
        print "  {:20s} | {:5s} : {:5s} : {:5s} : {:5s} : {:6s} : {:5s} |  | {:5s} : {:5s} : {:6s} : {:5s} |".format('Event', 'Nref', 'Nsys', 'P', 'R', 'F1', 'ER', 'Nref', 'Nsys', 'F1', 'ER')
        print "  ---------------------+-------+-------+-------+--------+-------+  +-------+-------+--------+-------+"
        seg_Nref = 0
        seg_Nsys = 0
        ...
        for event_label in sorted(overall_metrics_per_scene[scene_label]['segment_based_metrics']['class_wise']):
            print "  {:20s} | {:5d} : {:5d} : {:5.3f} : {:5.3f} : {:4.1f} % : {:5.2f} |  | {:5d} : {:5d} : {:4.1f} % : {:5.2f} |".format(event_label,
                                                                        ...
                                                                        overall_metrics_per_scene[scene_label]['segment_based_metrics']['class_wise'][event_label]['Pre'],
                                                                        overall_metrics_per_scene[scene_label]['segment_based_metrics']['class_wise'][event_label]['Rec'],
        ...
        print "  ---------------------+-------+-------+--------+-------+  +-------+-------+--------+-------+"
        print "  {:20s} | {:5d} : {:5d} : {:30s} |  | {:5d} : {:5d} : {:14s} |".format('Sum',
                                                                        seg_Nref,
                                                                        seg_Nsys,
                                                                        '',
                                                                        event_Nref,
